...
const Hero = () => {
    ...
    return slides.map((slide) => {
      return (
        <div
          key={slide.id}
          className={`slide ${
            activeSlide < slide.id ? "nextSlide" : "lastSlide"
          } ${activeSlide === slide.id && "activeSlide"} `}
        >
          <div className="flex flex-col px-4 pt-2 max-w-lg ">
            <h1 className="text-black text-5xl md:text-6xl font-extrabold mb-4">
              {slide.title}
            </h1>
            <Link
              to="/catalog"
              className="w-48 py-4 text-center bg-gray-900 text-gray-200 "
            >
              {" "}
              SHOP NOW
            </Link>
  ...
  return (
    <section className="relative bg-stone-300 py-8 slider">
      ...
      <div className="absolute bottom-8 w-full flex items-center justify-center gap-4">
        <button
          onClick={() => jump(0)}
          className={`${setDotWidth(
            0
          )} h-2 rounded-full border border-gray-700 transition-all ease `}
        ></button>
        <button
          onClick={() => jump(1)}
          className={`${setDotWidth(
