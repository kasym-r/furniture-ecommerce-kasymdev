...
const Product = () => {
  ...
  const mayLikeProds = DATA.filter(
    (pro) => pro.category === prod.category
  ).slice(0, 4);
    ...
    <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-4 min-h-screen p-4 md:pt-20 lg:p-8 ">
      ...
      <div className="flex flex-col items-start w-full gap-1 md:gap-4 h-4/5 lg:mt-8 px-2 lg:px-4">
        ...
        <div className="flex items-center gap-4 mb-2">
          ...
          >
            <i className="fas fa-heart mr-2 text-2xl cursor-pointer"></i> ADD TO
            WISHLIST
          </p>
          <button
            onClick={() => dispatch(addToCart(prod))}
            className="bg-gray-900 flex-1 text-sm text-gray-200 px-3 p-2"
          >
            ADD TO CART
          </button>
        </div>
        ...
        <Accordion
          content={"Additional inforamtion and details about the product !"}
        />
      </div>
