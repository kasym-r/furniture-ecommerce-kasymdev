    ...
    <div className="min-h-screen w-full py-8 px-2 md:px-8">
      ...
      {wishlistProds.length === 0 ? (
        <div className="w-full pt-24 flex justify-center items-center flex-col gap-6 ">
          <h1 className="text-center text-4xl font-extrabold text-red-400">
            YOUR WISHLIST IS STILL EMPTY <i className="fas fa-frown"></i>{" "}
          </h1>
          <p className="text-green-400 text-center ">
            Looks like you haven't seen our catalog
          </p>
          <Link to="/catalog" className="py-2 bg-green-400 w-64">
            BACK TO CATALOG
          </Link>
        </div>
      ) : (
            ...
            <div key={prod.id} className="py-5 sm:py-8 md:px-20 shadow mb-2">
              <div className="flex flex-wrap gap-4 lg:gap-6 sm:py-2.5">
                ...
                <div className="w-full sm:w-auto flex md:flex-col justify-between items-center border-t sm:border-none pt-4 sm:pt-0">
                  <button
                    onClick={() => moveTtocartHandle(prod)}
                    className="text-green-500 mx-2 hover:text-indigo-600 text-lg font-semibold select-none transition duration-100"
                  >
                    move to cart
                  </button>
                  <button
                    onClick={() => dispatch(removeFromWishlist(prod))}
                    className="text-red-500 mx-2 hover:text-red-800  text-lg font-semibold select-none transition duration-100"
                  >
                    delete
                  </button>
                </div>
